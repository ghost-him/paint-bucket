...
两种用法，同一套算法（同一张图的输出逐字节相同）：

- **桌面版**：图形界面，左右对比着调参数、滚轮放大看噪点、拖文件夹批处理，还能打包成一个 exe 发给别人
  ——见[「三、使用 / 1. 桌面版」](#1-桌面版)。
- **命令行**：`uv run celclean clean 图.png`——见[「三、使用 / 2. 命令行」](#2-命令行)。
...
需要 [uv](https://docs.astral.sh/uv/)（一个 Python 包管理器，装一次就行）。核心算法除了 numpy 和 Pillow 没有别的
依赖，不需要 GPU，不下载任何模型；桌面版额外装 Qt（PySide6）。

```bash
# 命令行版（只要 numpy + Pillow）
uv sync
...
```bash
uv run --extra gui celclean-gui              # 打开窗口
uv run --extra gui celclean-gui 你的图.png    # 直接打开一张图
```

窗口长这样：左边是参数，右边是**原图 / 清理后**两栏，滚轮缩放、按住拖动、双击回到适应窗口，两栏默认同步。

几件值得知道的事：

| 功能 | 怎么用 |
|---|---|
| **看噪点到底洗完没有** | 打开工具栏的 **「噪点放大」**：它把每个像素与它邻域平均值的差（单位就是色阶）放大后显示。平坦区会变成灰色底上的颗粒，颗粒的**幅度就是真实的噪声幅度**，所以左右两栏可以直接比——样图上平坦块纹理从 **0.489 色阶降到 0.168**，同一个增益下肉眼差别很明显 |
| 换个参数马上看效果 | **「自动预览」**默认开：改完参数 0.6 秒后自动重跑一次（样图一次约 3 秒），跑的时候界面不卡；中途又改了参数，只会保留最新的一次 |
| 批量处理 | 「批处理…」选文件夹（可含子文件夹），按当前参数全部处理；单个文件出错会跳过并汇总报告，不会中断 |
| 拖放 | 往窗口里拖图片 = 打开；拖多个文件或文件夹 = 进批处理 |
| 保存 | 「保存结果」写 `xxx-clean.png` 到原图旁边；「结果另存为…」选路径。保存的图片**不含**任何预览增强 |
| **换纯色背景** | 「透明通道 → 换成纯色背景」+ 选色（点色块或直接填 `#rrggbb`）：透明部分按 alpha 合成到该颜色上，输出不再有透明通道。半透明的抗锯齿边缘是逐像素混合的，**不会出现黑边** |
| 查看数字 | 「工具 → 测量指标」给出验收协议的全部数字（局部起伏、最坏改动、合成口径…），可一键复制 |
| 快捷键 | `Ctrl+O` 打开、`Ctrl+B` 批处理、`Ctrl+S` 保存、`Ctrl+R` 重新清理、`Ctrl+0/1` 适应窗口/1:1、`Ctrl+2/3/4` 并排/仅原图/仅清理后 |

#### 打包成单文件 exe（可以直接发给别人）

```bash
uv run --extra gui --group pack pyinstaller --noconfirm --clean packaging/celclean-gui.spec
# 产物：dist/celclean-gui.exe（Windows x64，约 66 MB，对方不需要装 Python）
```

单文件版启动时要把自己解包到临时目录，所以**首次出现窗口大约要 3 秒**（之后界面本身不慢）。验证一个构建是否完好：

```bash
dist/celclean-gui.exe --selftest --selftest-out selftest.json --out-dir build-check
```

它会真的开窗口、真的跑一遍清理、把结果与截图写进 `build-check/`，并在 JSON 里报告 `ok: true`。

这个 exe **不依赖 Python**（Qt 与解释器都打进去了）：直接发给别人用就行，也可以作为 GitHub Release 的附件挂着
供人下载（`dist/` 已加进 `.gitignore`，不会进仓库）。

#### 绿色（便携）版

打包版**默认就是绿色版**：配置存在 exe 旁边的 `celclean.ini` 里，不写注册表、也不建 AppData 目录。
所以整个软件就是**一个 exe** —— 拷到 U 盘、丢到别人机器上双击就能用，删掉不留痕
（运行时只在系统临时目录里解包自己，退出即清理）。

| 情况 | 配置存哪 |
|---|---|
| 打包版放在**可写**目录（默认） | 旁边的 `celclean.ini` |
| 打包版放在**不可写**目录（如 `C:\Program Files`） | 自动退回注册表 `HKCU\Software\celclean\celclean-gui`，不会报错 |
| 想强制便携 | `celclean-gui.exe --portable`，或设 `CELCLEAN_PORTABLE=1` |
| 从源码运行（`uv run celclean-gui`） | 默认写注册表（开发方便）；加 `--portable` 就在仓库根目录生成 `celclean.ini` |

配置里只有参数面板的取值和窗口布局；图片永远只在你指定的路径读写。「帮助 → 关于」里会写明当前用的是哪一个。
...
| `--alpha keep` / `--alpha flatten` | `normalize` | `keep` = 完全不动 alpha（输出仍保留透明）；`flatten` = 把整图按 alpha 合成到指定纯色上，**输出变成不含 alpha 通道的 RGB** |
| `--bg-color "#rrggbb"` | `#ffffff` | 只在 `--alpha flatten` 时生效（同 `--aa-band` 之于 `--snap`）。也接受 `white` / `black` / `#f00` |
...
- 桌面版与打包：`src/celclean/gui/`（界面、参数面板、预览、后台任务）+ `packaging/`（图标、PyInstaller 配置）
